...
def logged_in_users_count():
    ...
    extract_uuid = lambda line: next(
        iter(re.findall(r"^\[([0-9a-zA-Z-]+)\]", line)), None
    )
    ...
    try:
        ...
        while True:
            ...
            if user_has_logged_in and liuc == 0:
                ...
                if (
                    user_logged_out_time
                    and time.time() > user_logged_out_time + LAST_LOGOUT_TIMEOUT
                ):
                ...
                # No one is logged in and we have not timed out yet
                    ...
                    logging.debug(
                        f"All users logged out! Timing out in {LAST_LOGOUT_TIMEOUT} seconds"
                    )
                    user_logged_out_time = time.time()
                # No one is logged in and we have set a timeout
                else:
                    logging.debug(
                        "All users logged out! Timing out soon if no one logs back in!"
                    )
                ...
                if user_has_logged_in:
                    logging.debug(
                        f"{liuc} user(s) are still logged in! Keeping session alive..."
                    )
                    user_logged_out_time = None
                else:
...
class AutograderHarness(object):
    ...
    def run_autograder(self):
        ...
        with open("/autograder/results/stdout", "w") as stdoutf:
            ...
            exit_status, elapsed_time, timed_out = timeout_process(
                p, self.timeout_seconds
            )
    ...
    def run_sshd(self):
        ...
        v1p0p1 = [1, 0, 1]
        if is_at_least_version(ssh_wrapper_version, v1p0p1) and is_at_least_version(
            sshd_subprocess_file_version, v1p0p1
        ):
        ...
        # We are using an out of date SSH Wrapper.
            ...
            with open("/etc/motd", "ab") as f:
                ...
                f.write(
                    b"[Warning] Your base image has an update avaliable to improves ssh sessions!\n"
                )
                f.write(
                    b"[Info] Please rebuild your autograder to get the latest updates.\n"
                )
                f.write(
                    b"++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
                )
